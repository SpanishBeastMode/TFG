...
class Solver():
    def __init__(self, inputs, initializer, Ta=20, num_reads=1000):
        ...
        bqm = BinaryQuadraticModel.from_ising(*[model[k] for k in ['biases', 'couplings', 'offset']])
        ...
        if via == 'flux_biases':
            ...
            params['flux_biases'] = self._choose_flux_bias_for_qbts(self.initialized_multiplier.qbt_values)
        ...
        self.params = params
        self.schedule_ptn = lambda Sp, Tp, Ta=None: [[Ta, Sp], [Tp + Ta, Sp], [Tp + Ta * 2, 1]]
    ...
    def _choose_flux_bias_for_qbts(self, qbt_values, strength=1000):
        ...
        NUM_QUBITS = 5760
        input_qbts_flux_bias = {qbt: spin * PHI0 * strength for qbt, spin in qbt_values.items()}
    ...
    def parse_sampleset(self, sampleset):
        ...
        for sample, energy, num_occurrences in sampleset.data(['sample', 'energy', 'num_occurrences']):
            sample = {**sample, **self.initialized_multiplier.qbt_values} if self.via == 'api' or (self.via == 'adhoc_encoding' and not self.ancillar_enhanced) else sample
            (A, B, out), (truth_of_cfas, truth_of_chains, truth_of_zero_values), cfa_excitations = self.initialized_multiplier.parse_a_sample_of_all_qbt_values(sample)
            statistics[(A, B, out)][(round(energy, 3), (A*B==out), truth_of_cfas, truth_of_chains)] += num_occurrences

            for i in range(w):
                for j in range(l):
                    total_excitations[i][j] += num_occurrences * cfa_excitations[i][j]

        return statistics, total_excitations


    def print_statistics(self, statistics, total_excitations):
        sresult = f'inputs: {self.inputs["out"]}/{self.params["num_reads"]} samples\n'

        fmt = lambda slist: '{:>30} {:>10} {:>10} {:>15} {:>15} {:>10}\n'.format(*slist)

        sresult += fmt(['[output]', '[energy]', '[A*B=out]', '[truth of cfas]', '[truth of chains]', '[num]'])
    ...
    def _read_sampleset(self, fp, old_fp=None):
        try:
            with open(f'{fp}.json', 'r') as file:
                sampleset = SampleSet.from_serializable(json.loads(file.read()))
        except:
            if not old_fp:
                return None
            ...
            if f'{relative_fp}.txt' in os.listdir(os.path.dirname(old_fp)):
                ...
                with open(f'{fp}.json', 'w') as file:
                    file.write(json.dumps(sampleset.to_serializable(), indent=4))
    ...
    def _run(self, fp=None, params=None, old_fp=None):
        sampleset = self._read_sampleset(fp, old_fp=old_fp)
    ...
    def forward_annealing(self, Ta, anneal_dir=None, use_old_data=None):
        ...
        if use_old_data:
            out = self.inputs['out']
            filename = f'FW{Ta}us_factoring_{out}_with_{self.initialized_multiplier.length}*{self.initialized_multiplier.width}bit_of_CFA_{self.initialized_multiplier.cfa_index}_symmetric_None'
            old_fp, statistic_fp = [os.path.join(anneal_dir, f'{prefix}{filename}') for prefix in ['sampleset_', '']]
    ...
    def annealing_enhanced_by_pause(self, params, Tp=1, Sp_list=None, Ta=10, anneal_direction=None, anneal_dir=None, use_old_data=None, rounds=None):
        current_dir = os.path.join(anneal_dir, f'pause_of_Tp={Tp}us')

        if use_old_data:
            old_dir = os.path.join(anneal_dir, f'pause_for_{Tp}us')
            if os.path.exists(old_dir):
                shutil.move(old_dir, current_dir)
        ...
        for Sp in Sp_list:
            ...
            fp = os.path.join(current_dir, f'factoring_{self.inputs["out"]}_via_{sche}')
            old_fp = None

            if use_old_data:
                out = self.inputs['out']
                l, w = self.initialized_multiplier.length, self.initialized_multiplier.width
                cfa_index = self.initialized_multiplier.cfa_index
                if self.initialized_multiplier.length == 12 and rounds == 1:
                    filename = f'RV20us_factoring_{out}_the_first_sample-10.167_pause_at{Sp}_for{Tp}us_with_{l}*{w}bit_of_CFA_{cfa_index}_symmetric_None'
                else:
                    filename = f'iteration{rounds}-RV{Ta}us_factoring_{out}_pause_at{Sp}_for{Tp}us_with_{l}*{w}bit_of_CFA_{cfa_index}_symmetric_None'
                old_fp, statistic_fp = [os.path.join(current_dir, f'{prefix}{filename}') for prefix in ['sampleset_', '']]
                statistic_fp = statistic_fp + '.txt'
                if os.path.exists(statistic_fp):
                    os.remove(statistic_fp)
                
            # params = copy.deepcopy(self.params)
            params['anneal_schedule'] =  [schedule_from, *self.schedule_ptn(Sp, Tp, Ta)]

            print(f'\tSp={Sp}|Tp={Tp}us: \n\t\tfp = {fp}, \n\t\tparams["anneal_schedule"] = {params["anneal_schedule"]}')
            rs[Sp] = self._run(fp, params, old_fp=old_fp)
        return rs
    ...
    def run_FW_for_diff_Tas(self, Ta_list=[10, 30, 50, 100], **kwargs):
        ...
        results = {}
        for Ta in Ta_list:
            results[Ta] = self.forward_annealing(Ta, anneal_dir=anneal_dir, **kwargs)
    ...
    def run_pFW_for_diff_Tps(self, Tp_list=[1, 10, 30, 50, 100], Sp_list=None, anneal_direction='forward', Ta=10):
        ...
        anneal_dir = os.path.join(self.data_path, f'{anneal_direction}_annealing_enhanced_by_pause', f'with_Ta={Ta}us')
    ...
    def _get_global_minima_from_pure_FW(self, Ta_list=[10, 30, 50, 100], **kwargs):
        ...
        for ta, representatives in sorted(results.items(), key=lambda item: item[0]):
            ...
            optima = hamming_distance(minima.sample, grdstate.sample) if grdstate else minima.energy
    ...
    def _choose_minima_for_next_pRV_guided_by_moves(self, fminima, Sp_results, moves_list, bound=300):
        ...
        if not moves_list:
            if all([moves >= bound for moves, _ in reduced_Sp_results.values()]):
                minimal_moves = None
                ordered_Sp_results = sorted(reduced_Sp_results.items(), key=lambda item: item[1][0])
    ...
    def _get_minTp_to_reach_minima(self, Tp_results, subject='energy'):
        ...
        for tp, sp_results in Tp_results.items():
            for sp, representatives in sp_results.items():
                ...
                optima = hamming_distance(minima.sample, grdstate.sample) if subject =='distance' and grdstate else minima.energy
    ...
    def _choose_initial_state_for_pRV(self, pause_enhanced=False, Tp_list=None, Sp_list=None, Ta_list=[10], **kwargs):
        if pause_enhanced:
            ...
            to_dir = f'from_pFW_(Ta={ta}us, Tp={tp}us, Sp={sp}, minima={round(minima.energy, 3)})'
        else:
    ...
    def run_pRV_for_diff_Tps(self, pause_enhanced=False,
        ...
        from_minima, to_dir = self._choose_initial_state_for_pRV(pause_enhanced, Tp_list=Tp_list, Sp_list=Sp_list)
        ...
        results = {}
        for Tp in Tp_list:
            results[Tp] = self.annealing_enhanced_by_pause(params, Tp=Tp, Sp_list=Sp_list, Ta=Ta, anneal_direction=anneal_direction, anneal_dir=anneal_dir)
    ...
    def run_BFS_based_iterative_reverse_annealing(self, pause_enhanced=False, 
                                                        ...
                                                        use_old_data=True):
        anneal_dir = os.path.join(self.data_path, 'BFS_based_iterated_reverse_annealing')

        l, w = self.initialized_multiplier.length, self.initialized_multiplier.width
        hard_energies = {1: 8, 2: 6, 3: 2, 4: 0} if l == 16 else {1: 8, 2: 6, 3: 6, 4: 0} if l == 14 else {}    # tunable for the choice of IRV's initial state
        tp_list = [i for i in range(1, 10, 2)] if l == 16 else [i for i in range(1, 10, 2)] + [20, 100] if l == 14 else copy.deepcopy(Tp_list)    # tunable for each iteration

        from_minima, to_relative_dir = self._choose_initial_state_for_pRV(pause_enhanced, Tp_list, Sp_list, [Ta], use_old_data=use_old_data)
        ...
        rounds = 1
        iterative_dir = os.path.join(anneal_dir, f'IRV{rounds}_' + to_relative_dir)
        kwargs = {
            'Sp_list': Sp_list,
            'Ta': Ta,
        ...
        while tp_list:
            ...
            colors = plt.colormaps['viridis'](np.linspace(0, 1, len(initial_state_list)))
            colors = colors[::-1]    
            ...
            for initial_state in initial_state_list[::-1]:
                ...
                if rounds > 1:
                    to_relative_dir = f'from_a_low_energy_state-(e={fe}, median_moves={moves})'

                to_abs_dir = os.path.join(iterative_dir, '' if rounds == 1 else to_relative_dir)

                kwargs['anneal_dir'] = os.path.join(to_abs_dir, f'with_Ta={Ta}us')
                ...
                iterative_results_for_initial_states[(fe, moves)][tp] = (minima, Sp_results)
                # if rounds > 1:
                ...
                if te < round(MINIMAs[rounds-1][-1].energy):
                    ...
                    rounds += 1
                    iterative_dir = os.path.join(to_abs_dir, f'IRV{rounds}_from_the_IRV{rounds-1}(Tp={tp}us, Sp={sp})')
                    sp_fp = os.path.join(kwargs['anneal_dir'], f'pause_of_Tp={tp}us', f'factoring_{self.inputs["out"]}_via_(Ta={Ta}us, Tp={tp}us, Sp={sp})')
                    initial_state_list = self._choose_a_lower_energy_space_for_initial_states(rounds, initial_state, sp_fp, hard_energies=hard_energies)
                    if rounds == 4:
                        initial_state_list = initial_state_list[0:1]

                    if l == 16:
                        tp_list =  [1, 10, 50, 70, 100] if rounds >= 3 else copy.deepcopy(Tp_list)
                    elif l == 14:
                        tp_list = [1, 10, 50] if rounds >= 3 else copy.deepcopy(Tp_list)
                    else:
    ...
    def _recursive_pRV(self, rounds, from_minima, recursive_dir, Tp_list=None, Ta=None, moves_list=None,
        ...
        results = {}
        for Tp in Tp_list: 
            Sp_results = self.annealing_enhanced_by_pause(params, Tp=Tp, Ta=Ta, anneal_dir=anneal_dir, **kwargs)
    ...
    def gready_pRV(self, pause_enhanced=False, Tp_list=[1, 10, 30, 50, 100], Sp_list=None, Ta=10, anneal_direction='reverse', filtered=False):
        ...
        recursive_dir = os.path.join(self.data_path, f'{anneal_direction}_annealing_enhanced_by_pause')

        out = self.inputs['out']
        self.reversed_grdstate_path = os.path.join(recursive_dir, f'factoring_{out}_reversed_grdstate_path.json')

        from_minima, to_dir = self._choose_initial_state_for_pRV(pause_enhanced, Tp_list, Sp_list)
        ...
        while True:
            ...
            (tp, sp), minima = self._recursive_pRV(rounds, from_minima, recursive_dir, **kwargs)
            ...
            abbr = 'pRV'
            recursive_dir = os.path.join(recursive_dir, f'from_{abbr}_(Ta={Ta}us, Tp={tp}us, Sp={sp}, minima={round(energy, 3)})')
        return from_minima
    

    def _find_reversed_grdstate_path(self):
        anneal_approaches = ['forward_annealing'] + [f'{anneal_direction}_annealing_enhanced_by_pause' for anneal_direction in ['forward', 'reverse']]
    ...
    def _plot_energies_against_Ta(self, ax, results):
        ...
        for i, k in enumerate(ks):
            elist = [representatives[k][1] for representatives in results.values()]
    ...
    def _plot_distances_against_Ta(self, ax, results, wrpt_minima, **kwargs):
        ...
        Ta_list = list(results.keys())
        for i, k in enumerate(ks):
            distance_list = [hamming_distance(representatives[k].sample, minima.sample) for representatives in results.values()]
            label, marker = self._choose_plot_style_for_samples(int(k))
            ax.plot(Ta_list, distance_list, marker, label=label, color=colors[i])
            if k != 0: continue
    ...
    def _plot_energies_against_Sp(self, ax, results, filtered=False):
        ...
        colors = plt.colormaps['viridis'](np.linspace(0, 1, len(results.items())))
        colors = colors[::-1]    
        ...
        for Tp, rs in results.items():
            ...
            for k in ks:
                elist = [representatives[k][1] for representatives in rs.values()]
                label, marker = self._choose_plot_style_for_samples(int(k))
                ax.plot(Sp_list, elist, marker, label=f'(Tp={Tp}us, {label})', color=colors[i])
                if k != 0: continue
    ...
    def _plot_distances_against_Sp(self, ax, results, wrpt_minima=None, initialstate_far_from_grdstate=None, filtered=False):
        ...
        for Tp, rs in results.items():
            ...
            for k in ks:
                distance_list = [hamming_distance(representatives[k].sample, minima.sample) for representatives in rs.values()]
                label, marker = self._choose_plot_style_for_samples(int(k))
                ax.plot(Sp_list, distance_list, marker, label=f'(Tp={Tp}us, {label})', color=colors[i])
                if k != 0: continue
        ...
        if initialstate_far_from_grdstate:
            ax.axhline(y=initialstate_far_from_grdstate, color='red', linestyle='dotted')
    ...
    def _plot_results_wrpt(self, fp, results, title, wrpt_minima=None, against='Sp', **kwargs):
        ...
        if wrpt_minima:
            cols = 2
            fig, axs = plt.subplots(1, cols, figsize=(16, 6), constrained_layout=True)
            if against == 'Sp':
                eval(f'self._plot_energies_against_{against}')(axs[0], results, filtered=kwargs['filtered'])
            else:
                eval(f'self._plot_energies_against_{against}')(axs[0], results)
            eval(f'self._plot_distances_against_{against}')(axs[1], results, wrpt_minima, **kwargs)
        else:
    ...
    def plot_forward_annealing_results(self, results, anneal_dir): 
        ...
        self._plot_results_wrpt(fp, results, title, wrpt_minima=wrpt_minima, against='Ta')
    ...
    def plot_pause_enhanced_forward_annealing_results(self, results, anneal_dir, Ta):   
        ...
        title = f'Forward annealing enhanced by different pauses (Tp|Ta={Ta}us)'

        grdstate = self._find_groundstate()
        if grdstate:
            fp = os.path.join(anneal_dir, '(energy-distance)' + filename)
            self._plot_results_wrpt(fp, results, title, wrpt_minima=('grdstate', grdstate))
        else:
    ...
    def plot_pause_enhanced_reverse_annealing_results(self, results, anneal_dir, Ta, from_minima, 
        ...
        grdstate = self._find_groundstate()
        if grdstate:
            fp_wrpr_gd = os.path.join(anneal_dir, filtered_label + '(energy-distance)' + filename)
            kwargs['initialstate_far_from_grdstate'] = hamming_distance(minima.sample, grdstate.sample)
            self._plot_results_wrpt(fp_wrpr_gd, results, title, wrpt_minima=('grdstate', grdstate), **kwargs)
           
        fp = os.path.join(anneal_dir, filtered_label + '(energy-moves)' + filename)
        self._plot_results_wrpt(fp, results, title, wrpt_minima=('local_minima', minima), **kwargs)

        fp3D = os.path.join(anneal_dir, filtered_label + '(energy-moves-in-3D)' + filename)
        self._plot_results_wrpt_in_3D(fp3D, results, title, wrpt_minima=('local_minima', minima))
    ...
    def _plot_results_wrpt_in_3D(self, fp, results, title, wrpt_minima=None, against='Tp'):
        ...
        axs = [fig.add_subplot(1, cols, j+1, projection='3d') for j in range(cols)]
        ...
        for rs in results.values():
            for Sp, representatives in rs.items():
                for k in ks:
                    ...
                    distance = hamming_distance(kth_minima.sample, minima.sample)
                    dual_results[Sp][k].append((energy, distance))
                
        colors = plt.colormaps['viridis'](np.linspace(0, 1, len(dual_results.items()))) 
    ...
    def _plot_energies_against_Sp_for_all_initial_states_with_fixed_Tp(self, ax, iterative_results_per_tp, colors, k=0, prev_optima=None):
        ...
        for (fe, median_moves), (_, rs) in iterative_results_per_tp.items():
            Sp_list = list(rs.keys())
            min_e_list = [representatives[k].energy for representatives in rs.values()]
            color = colors[i]
            ax.plot(Sp_list, min_e_list, '.-', label=f'(moves={median_moves}|e={fe})', color=color)
    ...
    def _plot_distances_against_Sp_for_all_initial_states_with_fixed_Tp(self, ax, iterative_results_per_tp, colors, wrpt=None, k=0, prev_optima=None):
        ...
        for (fe, median_moves), (fsample, rs) in iterative_results_per_tp.items():
            ...
            distance_list = [hamming_distance(target, representatives[k].sample) for representatives in rs.values()]
            color=colors[i]
            ax.plot(Sp_list, distance_list, '.-', label=f'(moves={median_moves}|e={fe})', color=color)
            for Sp, dist in zip(Sp_list, distance_list):
                ax.annotate(dist, [Sp, dist])
            if grdstate:
        ...
        if grdstate and minima not in [round(fe, 3) for fe, _ in iterative_results_per_tp.keys()]:
            minima_dist = hamming_distance(prev_optima[-1].sample, grdstate.sample)
    ...
    def plot_BFS_based_iterative_results_per_tp(self, prev_optima, rounds, tp, iterative_results_per_tp, to_dir=None, colors=None):
        ...
        prefix, cols = ('(energy-moves-distance)', 3) if grdstate else ('(energy-moves)', 2)
        # prefix = '(for all Tps)'
        (Ta, Tp, Sp), optima = prev_optima
        e = round(optima.energy, 3)
        title = f'IRV{rounds}(Tp={tp}us) when initialized in different states obtained by IRV{rounds-1}((Tp={Tp}us, Sp={Sp}, minima={e})|Ta={Ta}us)'
        fp = os.path.join(to_dir, f'{prefix}{title}.png')

        if os.path.exists(fp):
            return

        fig, axs = plt.subplots(1, cols, figsize=(20, 6), constrained_layout=True)
        fig.suptitle(title)

        *args, kwargs = (iterative_results_per_tp, colors, {'prev_optima': prev_optima})
        for i, (ax, subject) in enumerate(zip(axs, ['energies', 'distances', 'distances'])):
            kwargs = copy.deepcopy(kwargs) 
            if i == 2:
                kwargs['wrpt'] = 'grdstate'
            eval(f'self._plot_{subject}_against_Sp_for_all_initial_states_with_fixed_Tp')(ax, *args, **kwargs)
    ...
    def _plot_energies_against_Sp_for_all_Tps_with_fixed_initial_state(self, ax, iterative_results_per_initial_state, k=0, prev_optima=None):
        ...
        colors = plt.colormaps['viridis'](np.linspace(0, 1, len(iterative_results_per_initial_state.keys())))
        colors = colors[::-1]    
    ...
    def _plot_distances_against_Sp_for_all_Tps_with_fixed_initial_state(self, ax, iterative_results_for_Tps, wrpt=None, k=0, prev_optima=None):
        ...
        colors = plt.colormaps['viridis'](np.linspace(0, 1, len(iterative_results_for_Tps.keys())))
        colors = colors[::-1]    
        ...
        for tp, (fsample, rs) in iterative_results_for_Tps.items():
            ...
            distance_list = [hamming_distance(target, representatives[k].sample) for representatives in rs.values()]
            color=colors[i]
            ax.plot(Sp_list, distance_list, '.-', label=f'Tp={tp}us', color=color)
            for Sp, dist in zip(Sp_list, distance_list):
                ax.annotate(dist, [Sp, dist])
            i += 1
    ...
    def _plot_BFS_based_iterative_results_per_initial_state(self, prev_optima, rounds, iterative_results_per_initial_state, to_dir=None):
        ...
        title = f'IRV{rounds}{prefix} when initialized in (moves={moves}|e={fe}) obtained by IRV{rounds-1}((Tp={Tp}us, Sp={Sp}, minima={e})|Ta={Ta}us)'
        fp = os.path.join(to_dir, f'{prefix}{title}.png')
        ...
        for i, (ax, subject) in enumerate(zip(axs, ['energies', 'distances', 'distances'])):
            ...
            if i == 2:
                kwargs['wrpt'] = 'grdstate'
            eval(f'self._plot_{subject}_against_Sp_for_all_Tps_with_fixed_initial_state')(ax, iterative_results_for_Tps, **kwargs)
    ...
    def plot_BFS_based_iterative_results_for_initial_states(self, iterative_results_for_initial_states, *args, **kwargs):
        for iterative_results_per_initial_state in iterative_results_for_initial_states.items():
            self._plot_BFS_based_iterative_results_per_initial_state(*args, iterative_results_per_initial_state, **kwargs)